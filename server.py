...
class HTTPHandler(SimpleHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        # Suprimir logs de HTTP para mantener la salida limpia
        pass

def start_http_server(port=8080):
    """Inicia un servidor HTTP simple para servir archivos estáticos"""
    try:
        server = HTTPServer(('0.0.0.0', port), HTTPHandler)
        logging.info(f"Servidor HTTP iniciado en 0.0.0.0:{port}")
        server.serve_forever()
    except Exception as e:
        logging.error(f"Error al iniciar servidor HTTP: {e}")

# Clase GameServer (sin cambios)
class GameServer:
    # [código existente de GameServer]
    def __init__(self):
    ...
    # Soportar CORS para conexiones WebSocket
    async def process_request(path, headers):
        if path == '/ws':
            # Esto es una conexión WebSocket, devuelve None para permitir continuar con el handshake
            return None
            
        # Devuelve una respuesta HTTP 200 para solicitudes normales
        if path == '/':
            return (200, [
        ...
        # Para verificaciones OPTIONS (preflight CORS)
        if path.startswith('/') and headers.get('Access-Control-Request-Method'):
            return (200, [
                ('Access-Control-Allow-Origin', '*'),
                ('Access-Control-Allow-Methods', 'GET, POST, OPTIONS'),
                ('Access-Control-Allow-Headers', 'Content-Type'),
        ...
        # Para cualquier otra ruta, devuelve 404
...
async def cleanup_games():
    """Limpia juegos abandonados periódicamente"""
    while True:
        try:
            ...
            for game_id, game in game_server.games.items():
                # Verificar si el juego tiene marca de tiempo de desconexión
                if "disconnectTime" in game and current_time > game["disconnectTime"]:
                    games_to_remove.append(game_id)
                
                # También eliminar juegos muy antiguos sin actividad (24 horas)
                elif current_time - game.get("creationTime", current_time) > 86400:  # 24 horas
                    games_to_remove.append(game_id)
            
            # Eliminar juegos abandonados
            for game_id in games_to_remove:
                if game_id in game_server.games:
                    logging.info(f"Eliminando juego abandonado: {game_id}")
            ...
            await asyncio.sleep(60)  # Esperar y reintentar
...
async def websocket_handler(websocket: WebSocketServerProtocol):
    ...
    try:
        ...
        logging.info(f"Conexión cerrada desde {client_info}: código {e.code}, razón: {e.reason}")
    except Exception as e:
...
async def main():
    ...
    port = int(os.environ.get('PORT', 10000))  # ¡IMPORTANTE! Usar el puerto 10000 por defecto
    ...
    # Configuración del servidor websocket con process_request para manejar CORS
    ...
    try:
        ...
        async with websockets.serve(
            ...
            host, 
            ws_port,
            ping_interval=None,      # Desactivar pings automáticos
            max_size=10*1024*1024    # 10MB máx por mensaje
        ):
            logging.info(f"Servidor WebSocket activo en {host}:{ws_port}")
            await asyncio.Future()  # Ejecutar indefinidamente
    except Exception as e:
        logging.error(f"Error iniciando servidor: {str(e)}")
        # Cancelar la tarea de limpieza si hay error
        cleanup_task.cancel()
        try:
            await cleanup_task
